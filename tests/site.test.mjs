...
test("every page is protected by server authentication without exposing access credentials", async () => {
  for (const file of await htmlFiles()) {
    ...
    assert.ok(
      html.indexOf("tradevisionpro-access-session-v4") < html.indexOf("assets/styles.css"),
      `restauration trop tardive : ${path.relative(DIST, file)}`,
    );
  }
...
test("volume six presents the complete money management course and a ten-question QCM", async () => {
  ...
  assert.equal((html.match(/callout--formula/g) || []).length, 5);
  ...
  assert.ok(html.includes("1. Gains pondérés"));
  assert.ok(html.includes("+0,05R par trade"));
  assert.ok(html.includes("Comment lire +0,05R"));
  assert.ok(html.includes("0R net"));
