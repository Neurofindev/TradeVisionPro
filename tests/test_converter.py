...
class ConverterOutputTests(unittest.TestCase):
    ...
    def test_volume_six_integrates_money_management_without_duplicate_exercises(self):
        ...
        self.assertIn("Kelly — f* = p − (1 − p) / b", rendered_text)
        self.assertIn("Taux d’équilibre = 1 ÷ (1 + gain moyen en R)", rendered_text)
        self.assertIn("Exemple actions, calculé étape par étape", rendered_text)
        self.assertIn("Quantité exécutable", rendered_text)
        self.assertIn("la capacité d’achat n’est pas le budget de risque", rendered_text)
        self.assertIn("1. Gains pondérés", rendered_text)
        self.assertIn("+0,05R par trade", rendered_text)
        self.assertIn("Comment lire +0,05R", rendered_text)
        self.assertIn("0R net", rendered_text)
    ...
    def test_volume_three_integrates_four_progressive_parts(self):
        ...
        self.assertEqual(len(self.v3["metadata"]["parts"]), 4)
        ...
        self.assertEqual(self.v3["metadata"]["parts"][3]["title"], "Les figures chartistes en trading")
        self.assertEqual(types.count("heading"), 191)
        self.assertEqual(types.count("lesson_note"), 13)
        self.assertEqual(types.count("figure"), 39)
        self.assertEqual(types.count("table"), 23)
        self.assertEqual(types.count("editorial_conclusion"), 2)
        self.assertEqual(self.v3["stats"]["chapterCount"], 47)
        ...
        self.assertIn("Les figures chartistes en trading", rendered_text)
        self.assertIn("1. Rappel des prérequis du Volume 3", rendered_text)
        self.assertIn("2. Double sommet et double creux", rendered_text)
        self.assertIn("Exemple guidé — un double sommet peut être valide mais peu intéressant", rendered_text)
        self.assertIn("Cas 4 — Aucun signal : refuser une figure forcée", rendered_text)
        self.assertNotIn("11. QCM d’auto-évaluation", rendered_text)
